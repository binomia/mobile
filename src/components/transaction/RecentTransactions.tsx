                                    ...
                                    <HStack alignItems={"center"} justifyContent={"space-between"} my={"10px"} borderRadius={10}>
                                        ...
                                        <VStack ml={"10px"} justifyContent={"center"}>
                                            {formatTransaction(data).showPayButton ?
                                                ...
                                                :
                                                <Heading
                                                    opacity={(data.status === "cancelled" || data.status === "suspicious") ? 0.5 : 1}
                                                    textDecorationLine={(data.status === "cancelled" || data.status === "suspicious") ? "line-through" : "none"}
                                ...
                                <Pressable bg={colors.lightGray} my={"5px"} borderRadius={10} px={"15px"} py={"10px"} key={`transactions(tgrtgnrhbfhrbgr)-${data.transactionId}-${index}-${data.transactionId}`} _pressed={{opacity: 0.5}} onPress={() => onOpenBottomSheet(data)}>
                                    <HStack alignItems={"center"} justifyContent={"space-between"} my={"10px"} borderRadius={10}>
                                        <HStack>
                                            {data.company.logo ?
                                                <Image borderRadius={100} resizeMode='contain' alt='logo-image'
                                                       w={scale(40)} h={scale(40)} source={{uri: data.company.logo}}/>
                                                :
                    ...
                    <VStack w={"100%"} h={height / 3} px={"20px"}>
                        ...
                        <VStack justifyContent={"center"} alignItems={"center"}>
                            <Heading textTransform={"capitalize"} fontSize={scale(20)}
                                     color={"white"}>Recientes</Heading>
                            <Text fontSize={scale(14)} w={"90%"} textAlign={"center"} color={"white"}>No hay
                                transacciones recentes para mostrar.</Text>
                        </VStack>
                    </VStack>
                )
            }
