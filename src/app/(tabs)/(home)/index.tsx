...
const HomeScreen: React.FC = () => {
    ...
    const [allowSend, setAllowSend] = useState(account?.allowSend);
    const [status, setStatus] = useState(account?.status || "active");

    const fetchAccount = async () => {
        try {
            const canRefresh = await allowReFetchAccount()
    ...
    const onRefresh = useCallback(async (showRefresh: boolean = true) => {
        try {
            ...
            const allowReFetch = await allowReFetchTransactions(true)
            if (allowReFetch) {
                await fetchAccount();
                    ...
                    <VStack w={"100%"} justifyContent={"center"} alignItems={"center"} borderRadius={"10px"}>
                        <VStack bg={colors.lightGray} p={"20px"} w={"100%"} justifyContent={"space-between"} borderRadius={"10px"} h={scale(160)}>
                            <VStack>
                                ...
                                <Heading fontSize={scale(28)} color={"white"}>{FORMAT_CURRENCY(accountBalance)}</Heading>
                            </VStack>
                            <HStack w={"100%"} alignItems={"center"} justifyContent={"space-between"}>
                                <Button
                                    opacity={allowSend ? 1 : 0.6}
                                    w={"49%"}
                                    bg={"darkGray"}
                                    mt={"20px"}
                                    borderRadius={"10px"}
